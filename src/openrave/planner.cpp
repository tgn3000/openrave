...
void PlannerParameters::copy(OPENRAVE_SHARED_PTR<PlannerParameters const> r)
{
    *this = *r;
}
...
bool PlannerParameters::endElement(const std::string& name)
{
    if( !!__pcurreader ) {
        if( __pcurreader->endElement(name) ) {
            OPENRAVE_SHARED_PTR<DummyXMLReader> pdummy = OPENRAVE_DYNAMIC_POINTER_CAST<DummyXMLReader>(__pcurreader);
            if( !!pdummy ) {
...
void PlannerParameters::SetRobotActiveJoints(RobotBasePtr robot)
{
    ...
    _distmetricfn = boost::bind(&SimpleDistanceMetric::Eval,OPENRAVE_SHARED_PTR<SimpleDistanceMetric>(new SimpleDistanceMetric(robot)),_1,_2);
    ...
    OPENRAVE_SHARED_PTR<SimpleNeighborhoodSampler> defaultsamplefn(new SimpleNeighborhoodSampler(pconfigsampler,_distmetricfn, _diffstatefn));
    ...
    OPENRAVE_SHARED_PTR<DynamicsCollisionConstraint> pcollision(new DynamicsCollisionConstraint(shared_parameters(), listCheckCollisions,0xffffffff&~CFO_CheckTimeBasedConstraints));
...
void PlannerParameters::SetConfigurationSpecification(EnvironmentBasePtr penv, const ConfigurationSpecification& spec)
{
    ...
    for(size_t igroup = 0; igroup < spec._vgroups.size(); ++igroup) {
        ...
        if( g.name.size() >= 12 && g.name.substr(0,12) == "joint_values" ) {
            ...
            OPENRAVE_SHARED_PTR<SimpleNeighborhoodSampler> defaultsamplefn(new SimpleNeighborhoodSampler(pconfigsampler,distmetricfns[isavegroup].first, diffstatefns[isavegroup].first));
...
class CustomPlannerCallbackData : public OPENRAVE_ENABLE_SHARED_FROM_THIS<CustomPlannerCallbackData>, public UserData
{
public:
...
typedef OPENRAVE_SHARED_PTR<CustomPlannerCallbackData> CustomPlannerCallbackDataPtr;
...
bool PlannerBase::InitPlan(RobotBasePtr pbase, std::istream& isParameters)
{
    ...
    OPENRAVE_SHARED_PTR<PlannerParameters> localparams(new PlannerParameters());
...
PlannerStatus PlannerBase::_ProcessPostPlanners(RobotBasePtr probot, TrajectoryBasePtr ptraj)
{
    ...
    FOREACHC(it,__listRegisteredCallbacks) {
        CustomPlannerCallbackDataPtr pitdata = OPENRAVE_DYNAMIC_POINTER_CAST<CustomPlannerCallbackData>(it->lock());
        if( !!pitdata) {
