...
public:
    PyManipulatorInfo() {
        ...
        _vChuckingDirection = bpndarray(boost::python::list());
...
public:
        ...
        object GetArmDOFValues()
        {
            if( _pmanip->GetArmDOF() == 0 ) {
                return bpndarray(boost::python::list());
            }
            vector<dReal> values;
        ...
        object FindIKSolutions(object oparam, int filteroptions, bool ikreturn=false, bool releasegil=false) const
        {
            ...
            if( ikreturn ) {
                ...
                return static_cast<bpndarray>(handle<>(pysolutions));
            }
        }
...
public:
    ...
    {
        if( _probot->GetActiveDOF() == 0 ) {
            return boost::python::make_tuple(bpndarray(boost::python::list()), bpndarray(boost::python::list())); // always need 2 since users can do lower, upper = GetDOFLimits()
        }
